...
def main(path_image, path_save, count):

    #Load model;
    dict_model = create_dict()
    model = Model(dict_model)
    model = model.load_weight(r"D:\RES\Resfes_FE\function_evalua\lan4_quang.pt")
    model = model.cuda()
    model.eval()
    ...
    inpainted_result = denormalize(inpainted_result)
    inpainted_result = tensor_to_numpy(inpainted_result)
    inpainted_result = convert_image_org(inpainted_result)

    #Convert tensor to numpy;
    x = tensor_to_numpy(x)
    image = image.permute(1, 2, 0)
    image = image.numpy()
    image_x = cv.imread(path_image)
    image_x = cv.resize(image_x, (256, 256))

    #Path file;
    a = "image_org" + str(count) + ".PNG"
    b = "image_mask" + str(count) + ".PNG"
    c = "image_res" + str(count) + ".PNG"
    image_org = os.path.join(path_save, a)
    image_mask = os.path.join(path_save, b)
    image_res = os.path.join(path_save, c)

    #Save image;
    cv.imwrite(filename=image_org, img=image_x)
    cv.imwrite(filename=image_mask, img=x*255)
    cv.imwrite(filename=image_res, img=inpainted_result*255)

# main(r"D:\RES\Resfes_FE\images\image_0.PNG", r"D:\a", 0)  